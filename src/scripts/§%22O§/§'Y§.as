package 
{
   import ;
   import LGDataEvent;
   import LGMathUtil;
   import ;
   import battlePanic.character.;
   import battlePanic.entity.;
   import battlePanic.entity.;
   import battlePanic.ui.techTree.UpgradeManager;
   import flash.geom.Point;
   
   public class  extends 
   {
       
      
      protected var _techLevel:int = 0;
      
      public var :Number = 0;
      
      public var :Number = 0;
      
      public var :Number = 0;
      
      public var :Number = 0;
      
      public var :Number = 100;
      
      public var :Number = 100;
      
      public var :Number = 1;
      
      public var :Number = 1;
      
      private var :int = 0;
      
      private var :UpgradeManager;
      
      private var LGMath:LGMathUtil;
      
      public var :Number = 0;
      
      public function ()
      {
         var _loc1_:Boolean = true;
         var _loc2_:Boolean = false;
         if(!_loc2_)
         {
            this. = UpgradeManager.getInstance();
            while(true)
            {
               this.LGMath = LGMathUtil.getInstance();
               loop1:
               while(_loc1_)
               {
                  while(true)
                  {
                     super();
                     if(_loc1_)
                     {
                        break;
                     }
                     continue loop1;
                  }
                  return;
               }
            }
         }
      }
      
      public function investResources(param1:Number, param2:Number) : void
      {
         var _loc9_:Boolean = false;
         var _loc10_:Boolean = true;
         push(param1);
         if(_loc10_ || Boolean(param2))
         {
            push(this..cheaperConstructionModifier);
            if(_loc10_ || _loc3_)
            {
               push(pop() * pop());
               if(_loc10_)
               {
                  addr36:
                  push(pop() - param1);
                  if(!(_loc9_ && Boolean(param2)))
                  {
                     push(pop());
                  }
               }
               var _loc3_:* = pop();
               push(param2);
               if(!_loc9_)
               {
                  push(this..cheaperConstructionModifier);
                  if(!(_loc9_ && Boolean(param2)))
                  {
                     addr67:
                     push(pop() * pop());
                     if(!_loc9_)
                     {
                        push(param2);
                     }
                     var _loc4_:* = pop();
                     push(param1);
                     if(_loc10_)
                     {
                        push(pop() + _loc3_);
                        if(_loc10_ || Boolean(param2))
                        {
                           addr80:
                           push(pop());
                        }
                        var _loc5_:* = pop();
                        push(param2);
                        if(_loc10_ || Boolean(param1))
                        {
                           push(pop() + _loc4_);
                           if(_loc10_)
                           {
                              addr94:
                              push(pop());
                           }
                           var _loc6_:* = pop();
                           push(this.);
                           if(!_loc9_)
                           {
                              push(pop() - this.);
                              if(!_loc9_)
                              {
                                 push(pop());
                              }
                           }
                           var _loc7_:* = pop();
                           push(this.);
                           if(!(_loc9_ && Boolean(param2)))
                           {
                              push(pop() - this.);
                              if(!(_loc9_ && Boolean(param2)))
                              {
                                 addr126:
                                 push(pop());
                              }
                              var _loc8_:* = pop();
                              if(_loc10_)
                              {
                                 push(_loc5_);
                                 loop0:
                                 while(true)
                                 {
                                    push(_loc7_);
                                    loop1:
                                    while(true)
                                    {
                                       if(pop() > pop())
                                       {
                                          _loc5_ = _loc7_;
                                          while(true)
                                          {
                                             push(param1);
                                             addr409:
                                             while(true)
                                             {
                                                push(_loc5_);
                                                addr410:
                                                while(true)
                                                {
                                                   if(pop() > pop())
                                                   {
                                                      while(true)
                                                      {
                                                         push(_loc5_);
                                                         addr412:
                                                         while(true)
                                                         {
                                                            push(pop());
                                                            addr420:
                                                            while(true)
                                                            {
                                                               param1 = pop();
                                                            }
                                                         }
                                                      }
                                                      addr411:
                                                   }
                                                }
                                             }
                                             addr368:
                                             push(this);
                                             push(this.);
                                             if(_loc10_ || Boolean(this))
                                             {
                                                push(pop() + _loc5_);
                                             }
                                             pop(). = pop();
                                             if(_loc9_)
                                             {
                                                continue;
                                             }
                                             push(this);
                                             push(this.);
                                             if(!(_loc9_ && Boolean(param2)))
                                             {
                                                push(pop() + _loc6_);
                                             }
                                             pop(). = pop();
                                             loop17:
                                             while(_loc10_ || Boolean(param2))
                                             {
                                                push(this);
                                                push(this.);
                                                if(_loc10_)
                                                {
                                                   push(pop() / this.);
                                                }
                                                pop(). = pop();
                                                push(this.techLevel == 0);
                                                loop18:
                                                while(true)
                                                {
                                                   push(pop());
                                                   loop19:
                                                   while(true)
                                                   {
                                                      if(!pop())
                                                      {
                                                         loop22:
                                                         while(true)
                                                         {
                                                            loop23:
                                                            while(true)
                                                            {
                                                               push(pop());
                                                               if(_loc9_)
                                                               {
                                                                  break;
                                                               }
                                                               if(!pop())
                                                               {
                                                                  loop26:
                                                                  while(pop())
                                                                  {
                                                                     if(_loc10_)
                                                                     {
                                                                        if(!_loc9_)
                                                                        {
                                                                           addr244:
                                                                           if(!(_loc9_ && Boolean(this)))
                                                                           {
                                                                              this.techLevel = 1;
                                                                              this. = 0;
                                                                              if(_loc10_ || Boolean(param1))
                                                                              {
                                                                                 break;
                                                                              }
                                                                              continue loop17;
                                                                              addr254:
                                                                           }
                                                                           else
                                                                           {
                                                                              while(_loc10_)
                                                                              {
                                                                              }
                                                                              loop12:
                                                                              while(true)
                                                                              {
                                                                                 push(_loc6_);
                                                                                 if(!(_loc9_ && Boolean(param2)))
                                                                                 {
                                                                                    while(!_loc9_)
                                                                                    {
                                                                                       if(!_loc9_)
                                                                                       {
                                                                                          push(pop());
                                                                                          while(true)
                                                                                          {
                                                                                             param2 = pop();
                                                                                             addr182:
                                                                                             if(_loc9_ && _loc3_)
                                                                                             {
                                                                                                continue;
                                                                                             }
                                                                                             if(_loc10_ || Boolean(param2))
                                                                                             {
                                                                                                addr393:
                                                                                                push(this.);
                                                                                                if(_loc10_ || Boolean(param1))
                                                                                                {
                                                                                                   addr212:
                                                                                                   if(!(_loc9_ && _loc3_))
                                                                                                   {
                                                                                                      if(!_loc9_)
                                                                                                      {
                                                                                                         if(_loc10_)
                                                                                                         {
                                                                                                            addr223:
                                                                                                            push(pop() >= pop());
                                                                                                            if(!(_loc9_ && Boolean(param2)))
                                                                                                            {
                                                                                                               if(!(_loc9_ && Boolean(this)))
                                                                                                               {
                                                                                                                  continue loop26;
                                                                                                               }
                                                                                                               continue loop23;
                                                                                                            }
                                                                                                            addr279:
                                                                                                            loop24:
                                                                                                            while(true)
                                                                                                            {
                                                                                                               if(_loc10_ || Boolean(param2))
                                                                                                               {
                                                                                                                  continue loop18;
                                                                                                               }
                                                                                                               addr302:
                                                                                                               while(true)
                                                                                                               {
                                                                                                                  pop();
                                                                                                                  break loop24;
                                                                                                               }
                                                                                                            }
                                                                                                            continue loop17;
                                                                                                         }
                                                                                                         continue loop1;
                                                                                                      }
                                                                                                   }
                                                                                                   while(true)
                                                                                                   {
                                                                                                      if(pop() > pop())
                                                                                                      {
                                                                                                         continue loop12;
                                                                                                      }
                                                                                                      addr361:
                                                                                                      while(true)
                                                                                                      {
                                                                                                         .(param1,param2);
                                                                                                      }
                                                                                                   }
                                                                                                   addr374:
                                                                                                }
                                                                                                else
                                                                                                {
                                                                                                   addr271:
                                                                                                   while(true)
                                                                                                   {
                                                                                                      if(_loc10_)
                                                                                                      {
                                                                                                         push(pop() >= pop());
                                                                                                         continue loop22;
                                                                                                      }
                                                                                                   }
                                                                                                   addr271:
                                                                                                }
                                                                                                while(true)
                                                                                                {
                                                                                                   if(pop() > pop())
                                                                                                   {
                                                                                                      push(_loc8_);
                                                                                                      if(!(_loc9_ && Boolean(param2)))
                                                                                                      {
                                                                                                         if(_loc10_)
                                                                                                         {
                                                                                                            push(pop());
                                                                                                            break loop22;
                                                                                                         }
                                                                                                         break;
                                                                                                      }
                                                                                                      break loop22;
                                                                                                   }
                                                                                                }
                                                                                                continue loop0;
                                                                                                addr393:
                                                                                             }
                                                                                          }
                                                                                          addr388:
                                                                                       }
                                                                                       else
                                                                                       {
                                                                                       }
                                                                                    }
                                                                                    addr383:
                                                                                 }
                                                                              }
                                                                              addr366:
                                                                           }
                                                                        }
                                                                        else
                                                                        {
                                                                           while(true)
                                                                           {
                                                                              push(this.LGMath);
                                                                              if(_loc9_ && _loc3_)
                                                                              {
                                                                                 break;
                                                                              }
                                                                              push(this.);
                                                                              if(!(_loc9_ && _loc3_))
                                                                              {
                                                                                 push(0.999);
                                                                                 if(_loc10_ || Boolean(param2))
                                                                                 {
                                                                                    push(pop().(pop(),pop()));
                                                                                    if(_loc10_ || Boolean(param1))
                                                                                    {
                                                                                    }
                                                                                    else
                                                                                    {
                                                                                       addr262:
                                                                                       while(_loc10_ || _loc3_)
                                                                                       {
                                                                                          push(this.);
                                                                                       }
                                                                                       break loop22;
                                                                                       addr262:
                                                                                    }
                                                                                 }
                                                                                 else
                                                                                 {
                                                                                    while(true)
                                                                                    {
                                                                                    }
                                                                                    addr261:
                                                                                 }
                                                                              }
                                                                              else
                                                                              {
                                                                                 while(true)
                                                                                 {
                                                                                 }
                                                                                 addr260:
                                                                              }
                                                                           }
                                                                           while(true)
                                                                           {
                                                                           }
                                                                           addr294:
                                                                           addr258:
                                                                        }
                                                                     }
                                                                  }
                                                                  return;
                                                               }
                                                            }
                                                            continue loop19;
                                                         }
                                                         while(true)
                                                         {
                                                            _loc6_ = pop();
                                                            push(param2);
                                                            if(_loc10_)
                                                            {
                                                               push(_loc6_);
                                                            }
                                                         }
                                                      }
                                                   }
                                                }
                                                while(!(_loc9_ && Boolean(param1)))
                                                {
                                                   while(true)
                                                   {
                                                   }
                                                }
                                             }
                                          }
                                       }
                                       while(true)
                                       {
                                       }
                                    }
                                 }
                              }
                           }
                        }
                     }
                  }
                  push(pop() - pop());
                  if(!_loc9_)
                  {
                     push(pop());
                  }
               }
            }
         }
      }
      
      public function (param1:int, param2:int, param3:int = -1) : void
      {
         var _loc4_:Boolean = true;
         var _loc5_:Boolean = false;
         push(param3);
         if(!(_loc5_ && Boolean(this)))
         {
            if(pop() == -1)
            {
               addr125:
               param3 = shared.BUILD_COST_PER_FRAME;
            }
            this. = param1;
            this. = param2;
            if(this. <= this.)
            {
               push(this);
               push(this.);
               if(_loc4_ || Boolean(this))
               {
                  push(pop() / this.);
                  if(!(_loc5_ && Boolean(this)))
                  {
                     push(pop() * param3);
                  }
               }
               pop(). = pop();
               loop0:
               while(!_loc5_)
               {
                  this. = param3;
                  if(!_loc4_)
                  {
                     continue;
                  }
                  if(!_loc5_)
                  {
                  }
                  else
                  {
                     addr91:
                     this. = param3;
                     while(true)
                     {
                        push(this);
                        push(this.);
                        if(!(_loc5_ && Boolean(param2)))
                        {
                           addr81:
                           push(pop() / this.);
                           if(!_loc5_)
                           {
                              addr79:
                              push(pop() * param3);
                           }
                           pop(). = pop();
                           break loop0;
                        }
                     }
                     addr94:
                  }
               }
               while(_loc5_)
               {
               }
               addr14:
               return;
            }
         }
      }
      
      public function get techLevel() : int
      {
         return this._techLevel;
      }
      
      public function set techLevel(param1:int) : void
      {
         var _loc2_:Boolean = false;
         var _loc3_:Boolean = true;
         if(!_loc2_)
         {
            this._techLevel = param1;
            loop0:
            while(true)
            {
               push(param1);
               addr70:
               loop1:
               while(true)
               {
                  push(pop() >= 0);
                  while(true)
                  {
                     if(!pop())
                     {
                        while(pop())
                        {
                           if(!(_loc2_ && _loc2_))
                           {
                              if(_loc3_)
                              {
                                 clip.gotoAndStop(param1 + 1);
                                 continue loop0;
                              }
                              continue loop1;
                           }
                           continue loop0;
                        }
                        addr17:
                        return;
                     }
                     while(true)
                     {
                        pop();
                        continue loop1;
                     }
                     addr30:
                     if(!(_loc3_ || Boolean(param1)))
                     {
                        continue;
                     }
                  }
               }
            }
         }
         while(true)
         {
            push(param1);
            if(!_loc2_)
            {
               push(pop() <= shared.TECH_LEVELS);
               if(_loc2_)
               {
                  continue;
               }
            }
         }
      }
      
      override public function setGraphicsFromClipName(param1:String, param2:String = null, param3:Boolean = false, param4:Boolean = false, param5:int = 1) : void
      {
         var _loc6_:Boolean = true;
         var _loc7_:Boolean = false;
         if(_loc6_)
         {
            super.setGraphicsFromClipName(param1,param2,param3,param4,param5);
         }
         do
         {
            clip.gotoAndStop(1);
         }
         while(!_loc6_);
         
      }
      
      public function (param1:int, param2:Number = 1, param3:Number = 1, param4:Number = 1) : 
      {
         var _loc12_:Boolean = false;
         var _loc13_:Boolean = true;
         var _loc9_: = null;
         var _loc5_:;
         push((_loc5_ = shared.characterFactory.definitionOfCombatantAtTechLevel(param1,this.techLevel)).);
         if(!_loc12_)
         {
            push(pop() * param2);
            if(_loc13_)
            {
               push(pop());
            }
         }
         var _loc6_:* = pop();
         push(_loc5_.);
         if(_loc13_)
         {
            push(pop() * param3);
            if(_loc13_ || Boolean(param1))
            {
               push(pop());
            }
         }
         var _loc7_:* = pop();
         var _loc8_:;
         push((_loc8_ = .(_loc6_,_loc7_)).);
         push((_loc8_ = .(_loc6_,_loc7_)).);
         if(_loc13_ || Boolean(this))
         {
            if(pop())
            {
               pop();
               push(_loc8_.);
               if(_loc13_ || Boolean(param3))
               {
               }
            }
            addr79:
            if(pop())
            {
               var _loc10_:*;
               push((_loc10_ = this).);
               if(_loc13_)
               {
                  push(pop() + 1);
               }
               var _loc11_:* = pop();
               if(_loc13_)
               {
                  _loc10_. = _loc11_;
               }
               push(this);
               push(Number(this.) / Number(shared.SPAWN_RATE));
               if(!_loc12_)
               {
                  push(pop() / param4);
               }
               pop(). = pop();
               push(this.);
               if(!(_loc12_ && Boolean(this)))
               {
                  push(param4);
                  if(_loc13_ || Boolean(param2))
                  {
                     push(pop() * shared.SPAWN_RATE);
                  }
                  if(pop() >= pop())
                  {
                     if(_loc13_)
                     {
                        this. = 0;
                        if(_loc13_ || Boolean(this))
                        {
                           loop6:
                           while(true)
                           {
                              shared.characterFactory.spawnGoodGuy(param1,-1,param2,param3);
                              if(_loc13_)
                              {
                                 push(this..massRecruitmentFlag);
                                 loop0:
                                 while(true)
                                 {
                                    push(pop());
                                    if(!(_loc12_ && Boolean(this)))
                                    {
                                       push(pop());
                                       loop7:
                                       while(pop())
                                       {
                                          if(_loc13_ || Boolean(param2))
                                          {
                                             while(true)
                                             {
                                                pop();
                                                if(true)
                                                {
                                                   addr280:
                                                   push(param1);
                                                   if(_loc13_)
                                                   {
                                                      addr192:
                                                      push(pop() == shared.characterFactory.KNIGHT);
                                                      break loop7;
                                                   }
                                                   if(pop() < shared.TECH_LEVELS)
                                                   {
                                                      if(!(_loc12_ && Boolean(this)))
                                                      {
                                                         addr290:
                                                         push((_loc10_ = this).techLevel);
                                                         if(_loc13_ || Boolean(this))
                                                         {
                                                            push(pop() + 1);
                                                         }
                                                         _loc11_ = pop();
                                                         if(_loc13_)
                                                         {
                                                            _loc10_.techLevel = _loc11_;
                                                         }
                                                         this.requestLevelUpEffect();
                                                         addr349:
                                                         while(true)
                                                         {
                                                            break loop0;
                                                         }
                                                         addr349:
                                                         addr310:
                                                      }
                                                      loop4:
                                                      while(true)
                                                      {
                                                         addr317:
                                                         loop3:
                                                         while(true)
                                                         {
                                                            push(this);
                                                            push(this.);
                                                            if(_loc13_ || Boolean(param1))
                                                            {
                                                               push(pop() / 100);
                                                            }
                                                            pop(). = pop();
                                                            if(!_loc13_)
                                                            {
                                                               while(true)
                                                               {
                                                                  if(_loc13_)
                                                                  {
                                                                     continue loop4;
                                                                  }
                                                                  continue loop3;
                                                               }
                                                               addr344:
                                                            }
                                                         }
                                                      }
                                                   }
                                                   break loop0;
                                                }
                                                continue loop6;
                                             }
                                             addr117:
                                          }
                                          break;
                                       }
                                       if(pop())
                                       {
                                          push((_loc8_ = .(_loc6_,_loc7_)).);
                                          if(!_loc12_)
                                          {
                                             push(pop());
                                             if(!_loc12_)
                                             {
                                                if(pop())
                                                {
                                                   if(!_loc12_)
                                                   {
                                                      addr214:
                                                      pop();
                                                      if(!(_loc12_ && Boolean(param3)))
                                                      {
                                                         push(_loc8_.);
                                                         if(_loc13_)
                                                         {
                                                            addr227:
                                                            addr226:
                                                            if(pop())
                                                            {
                                                            }
                                                            this. += shared.PERCENT_UPGRADE_PER_SPAWN;
                                                            if(this. >= 100 - shared.PERCENT_UPGRADE_PER_SPAWN * this..buildingAdvancements)
                                                            {
                                                               push(this.techLevel);
                                                            }
                                                         }
                                                      }
                                                      _loc9_ = shared.characterFactory.spawnGoodGuy(param1,-1,param2,param3);
                                                      _loc9_.x += 5;
                                                      if(_loc13_)
                                                      {
                                                         _loc9_.y += 5;
                                                         if(_loc13_)
                                                         {
                                                            this. += shared.PERCENT_UPGRADE_PER_SPAWN;
                                                         }
                                                      }
                                                   }
                                                }
                                             }
                                          }
                                       }
                                       addr109:
                                    }
                                 }
                                 addr130:
                                 while(true)
                                 {
                                    this. = 0;
                                 }
                                 addr100:
                              }
                           }
                        }
                     }
                  }
               }
            }
            addr312:
            return _loc8_;
         }
      }
      
      protected function requestLevelUpEffect() : void
      {
         var _loc1_:Boolean = true;
         var _loc2_:Boolean = false;
         if(_loc1_ || _loc2_)
         {
            shared.CENTRAL_DISPATCHER.dispatchEvent(new LGDataEvent("displayLevelUpEffect",{"position":new Point(x,y - 50)}));
         }
      }
   }
}
