package battlePanic.transition
{
   ...
   public class Transition extends Sprite
   {
      ...
      private static var _instance:battlePanic.transition.Transition;
      ...
      private var _currentView:battlePanic.transition.Transitionable = null;
      
      private var _previousView:battlePanic.transition.Transitionable = null;
      ...
      public static function getInstance() : battlePanic.transition.Transition
      {
         ...
         {
            push(_instance);
            if(_loc2_ || battlePanic.transition.Transition)
            {
               if(pop() == null)
               {
                  if(!_loc1_)
                  {
                     addr47:
                     _instance = new battlePanic.transition.Transition(new SingletonBlocker());
                  }
               }
               return _instance;
            }
      ...
      public function init() : *
      {
         ...
         while(true)
         {
            ...
            while(true)
            {
               ...
               while(true)
               {
                  this._startBitmapData = new BitmapData(this.shared.,this.shared.RENDER_SURFACE_HEIGHT);
                  while(!(_loc2_ && _loc1_))
                  {
                     this._destinationBitmapData = new BitmapData(this.shared.,this.shared.RENDER_SURFACE_HEIGHT);
      ...
      public function transition(param1:battlePanic.transition.Transitionable, param2:battlePanic.transition.Transitionable) : void
      {
         ...
         while(true)
         {
            ...
            while(true)
            {
               ...
               this.DisplayListUtil.(this._clip,true,true);
      ...
      private function transitionAnimationCompleteHandler() : void
      {
         ...
         if(_loc2_)
         {
            this.DisplayListUtil.(this._clip,false,true,true);
            loop0:
            while(true)
            {
      ...
      public function transitionTo(param1:battlePanic.transition.Transitionable) : void
      {
         var _loc2_:Boolean = true;
         var _loc3_:Boolean = false;
      ...
      public function makeOneViewVisible(param1:battlePanic.transition.Transitionable) : void
      {
         var _loc2_:Boolean = false;
         var _loc3_:Boolean = true;
         if(!(_loc2_ && _loc3_))
         ...
         for(_loc1_ in _loc3_)
         {
            ...
            while(true)
            {
               ...
               while(true)
               {
                  while(true)
                  {
                     LGDisplayListUtil.getInstance().(DisplayObjectContainer(_loc1_),false);
            ...
            while(true)
            {
               ...
               while(param2)
               {
                  if(!(_loc3_ && Boolean(this)))
                  {
                     while(true)
                     {
                        LGDisplayListUtil.getInstance().(DisplayObjectContainer(param1),false,true,true);
                     }
                     addr43:
                  }
                  while(true)
      ...
      public function set currentView(param1:MovieClip) : void
      {
         ...
         if(!_loc3_)
         {
            if(!(param1 is battlePanic.transition.Transitionable))
            {
               while(true)
               {
