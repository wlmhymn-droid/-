package battlePanic.ui
{
   ...
   public class GameOverScreen extends RevealableView implements Transitionable
   {
      ...
      private static var _instance:battlePanic.ui.GameOverScreen;
      ...
      private var _tryAgainButtonHomePosition:;
      
      private var _defeatStoreButtonHomePosition:;
      ...
      private var _toolTip:battlePanic.ui.TooltipScores;
            ...
            while(true)
            {
               ...
               while(true)
               {
                  ...
                  while(true)
                  {
                     ...
                     this._toolTip = new battlePanic.ui.TooltipScores();
                  ...
                  this._tryAgainButtonHomePosition = new (this._clip.defeatPage.tryAgainButton.x,this._clip.defeatPage.tryAgainButton.y);
                  this._defeatStoreButtonHomePosition = new (this._clip.defeatPage.storeButton.x,this._clip.defeatPage.storeButton.y);
               }
            }
         }
      }
      
      public static function getInstance() : battlePanic.ui.GameOverScreen
      {
         var _loc1_:Boolean = true;
         var _loc2_:Boolean = false;
         if(_loc1_ || battlePanic.ui.GameOverScreen)
         {
            push(_instance);
            if(_loc1_)
            {
               if(pop() == null)
               {
                  ...
                  {
                     addr46:
                     _instance = new battlePanic.ui.GameOverScreen(new SingletonBlocker());
                  }
               }
               return _instance;
            }
      ...
      public function showGameLose() : void
      {
         ...
         while(true)
         {
            ...
            do
            {
               ...
               while(true)
               {
                  pop().(this._clip.defeatPage,true,true,true);
                  while(_loc2_)
                  {
                     push(LGDisplayListUtil.getInstance());
                     if(!_loc3_)
                     {
                        pop().(this._clip.victoryPage,false,true,true);
                        if(!_loc3_)
                        {
                           continue loop1;
                        }
      ...
      public function showGameWin(param1:Boolean = false) : void
      {
         ...
         while(true)
         {
            ...
            while(true)
            {
               ...
               do
               {
                  pop().(this._clip.defeatPage,false,true,true);
                  push(LGDisplayListUtil.getInstance());
               }
               while(!(_loc2_ || _loc2_));
               
               pop().(this._clip.victoryPage,true,true,true);
               do
               {
      ...
      private function flattenVignette() : void
      {
         ...
         if(_loc4_)
         {
            ...
            do
            {
               LGDisplayListUtil.getInstance().(this._clip.vignette);
               do
               {
